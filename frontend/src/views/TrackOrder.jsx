  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-14 min-h-[70vh]">
      ...
      <div className="bg-white border border-ink-200 rounded-none p-6 mt-8 space-y-4">
        <input value={orderNum} onChange={(e) => setOrderNum(e.target.value.toUpperCase())} placeholder="PK-SNK-XXXXX" data-testid="tracking-search-input" className="w-full border border-ink-200 rounded-none px-4 py-3 outline-none focus:border-obsidian font-mono uppercase" />
        <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone used at checkout" className="w-full border border-ink-200 rounded-none px-4 py-3 outline-none focus:border-obsidian" />
        <button onClick={track} disabled={loading} data-testid="tracking-search-btn" className="w-full bg-obsidian text-white font-display font-bold uppercase tracking-wider py-4 rounded-none flex items-center justify-center gap-2 hover:bg-fire transition-colors"><Search size={17} /> {loading ? "Searching…" : "Track Order"}</button>
      </div>
