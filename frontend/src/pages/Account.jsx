    ...
    <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-10 min-h-[70vh]">
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        <aside className="lg:col-span-1">
          <nav className="flex lg:flex-col gap-1 overflow-x-auto no-scrollbar">
            {[["orders", "My Orders", Package], ["credit", "Store Credit", Gift], ["wishlist", "Wishlist", Heart], ["settings", "Settings", User]].map(([k, l, I]) => (
              <button key={k} onClick={() => setTab(k)} data-testid={`account-tab-${k}`} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-display font-bold uppercase text-sm tracking-tight whitespace-nowrap ${tab === k ? "bg-obsidian text-white" : "hover:bg-ink-100"}`}><I size={17} /> {l}{k === "credit" && credit.balance > 0 && <span className="ml-1 font-mono text-fire">{fmt(credit.balance)}</span>}</button>
            ))}
        ...
        <div className="lg:col-span-3">
          {tab === "orders" && (
            orders.length === 0 ? <EmptyState icon={Package} title="No orders yet" subtitle="Your order history will appear here." /> : (
              <div className="space-y-4">
                {orders.map((o) => (
                  <div key={o.id} data-testid={`account-order-${o.order_number}`} className="bg-white border border-ink-200 rounded-2xl p-5">
                    ...
                    <div className="flex justify-between items-center mt-3 pt-3 border-t border-ink-200">
                      ...
                      <div className="flex items-center gap-3">
                        {o.status === "delivered" && <button onClick={() => setReturnFor(o)} data-testid={`account-return-btn-${o.order_number}`} className="flex items-center gap-1.5 text-xs font-bold uppercase border border-ink-200 px-3 py-1.5 rounded-full hover:border-obsidian"><RotateCcw size={13} /> Return</button>}
          ...
          {tab === "credit" && (
            <div>
              <div className="bg-obsidian text-white rounded-2xl p-6 mb-5" data-testid="account-store-credit-balance">
              ...
              {credit.ledger.length === 0 ? <p className="text-ink-400 text-sm">No transactions yet.</p> : (
                <div className="space-y-2">
                  {credit.ledger.map((l) => (
                    <div key={l.id} className="flex justify-between items-center bg-white border border-ink-200 rounded-xl px-4 py-3">
      ...
      {returnFor && (
        <div className="fixed inset-0 bg-obsidian/60 z-50 grid place-items-center p-5" onClick={() => setReturnFor(null)}>
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={4} placeholder="Reason for return (e.g. size too small, defect)" data-testid="account-return-reason" className="w-full border border-ink-200 rounded-xl px-4 py-3 outline-none focus:border-obsidian" />
            <button onClick={submitReturn} data-testid="account-return-submit" className="w-full bg-obsidian text-white font-display font-bold uppercase py-3 rounded-full mt-4 hover:bg-fire transition-colors">Submit Request</button>
          </motion.div>
        </div>
      )}
    </div>
