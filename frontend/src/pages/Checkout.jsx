...
export default function Checkout() {
  ...
  if (cart.items.length === 0) {
    return <div className="max-w-lg mx-auto px-5 py-20 text-center"><h2 className="font-display text-2xl font-black uppercase">Your bag is empty</h2><button onClick={() => navigate("/")} className="mt-6 bg-obsidian text-white font-bold uppercase px-8 py-4 rounded-none">Continue Shopping</button></div>;
  }
  ...
  const Field = ({ label, k, type = "text", placeholder, testid, required }) => (
    <div>
      ...
      <input type={type} value={form[k]} onChange={set(k)} placeholder={placeholder} data-testid={testid} className="w-full mt-1.5 border border-ink-200 rounded-none px-4 py-3 outline-none focus:border-obsidian transition-colors" />
    </div>
  );
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        <form data-testid="checkout-form" onSubmit={(e) => e.preventDefault()} className="lg:col-span-2 space-y-8">
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.city} onChange={set("city")} data-testid="checkout-city-select" className="w-full mt-1.5 border border-ink-200 rounded-none px-4 py-3 outline-none focus:border-obsidian bg-white">
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                ...
                <button key={m.v} onClick={() => setPayment(m.v)} data-testid={`checkout-payment-method-${m.v}`} className={`w-full flex items-center gap-4 border-2 rounded-none p-4 text-left transition-colors ${payment === m.v ? "border-obsidian bg-white" : "border-ink-200"}`}>
                  <div className={`h-11 w-11 grid place-items-center rounded-none ${payment === m.v ? "bg-fire text-white" : "bg-ink-100"}`}><m.icon size={19} /></div>
            ...
            {payment === "WALLET" && <p className="text-xs text-ink-400 mt-3 bg-fire-subtle p-3 rounded-none">Note: Wallet payment is <b>MOCKED</b> for demo — order confirms instantly.</p>}
            {advanceRequired && (
        ...
        <div className="lg:sticky lg:top-28 lg:self-start bg-white rounded-none p-6 border border-ink-200 h-fit">
          ...
          <button onClick={placeOrder} disabled={placing} data-testid="checkout-submit-order-btn" className="w-full bg-obsidian text-white font-display font-bold uppercase tracking-wider py-4 rounded-none mt-5 hover:bg-fire transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
